...
"""
=============================================================================
 HIGH-THROUGHPUT MATERIAL SCREENING ENGINE - A2BB'O6 | ULTIMATE V8.2 (FIXED)
...
def main():
    ...
    with st.sidebar:
        ...
        target_a = st.number_input("Valeur cible 'a' (Å)", min_value=3.0, max_value=20.0, value=float(st.session_state.target_a), step=0.05, key='target_a')
        delta_a = st.slider("Marge ±Δa (Å)", 0.01, 3.0, value=float(st.session_state.delta_a), step=0.01, key='delta_a')
